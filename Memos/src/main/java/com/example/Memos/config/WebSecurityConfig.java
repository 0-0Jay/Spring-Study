...
public class WebSecurityConfig {
	...
	SecurityFilterChain apiFilterChain(HttpSecurity http) throws Exception {
		http.securityMatcher("/api/**").csrf(csrf -> csrf.disable())
				.authorizeHttpRequests(authorize -> authorize.requestMatchers("/api/join", "/api/signin", "/api/token")
	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		return http
				.authorizeHttpRequests(requests -> requests
						.requestMatchers(HttpMethod.GET, "/css/**", "/js/**", "/images/**", "/uploads/**").permitAll() // static
																														// files
						.requestMatchers("/signin", "/signout", "/join", "/").permitAll() // All method
						.anyRequest().authenticated()) // 나머지 모든 페이지는 로그인 해야 사용 가능
				.formLogin(login -> login.loginPage("/signin").defaultSuccessUrl("/memos").failureForwardUrl("/"))
				.logout(logout -> logout.logoutUrl("/signout").logoutSuccessUrl("/").invalidateHttpSession(true))
				.build();
	}
