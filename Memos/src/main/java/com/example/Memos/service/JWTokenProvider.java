...
public class JWTokenProvider {
	...
	public String createToken(User user, Duration expiredAt) {
		...
		return Jwts.builder().header().add(getHeader()).and().claims().issuedAt(now).issuer(jwtProperties.getIssuer())
				.subject(user.getUsername()).expiration(exp).add("role", user.getRole()).and()
	...
	public Authentication getAuthentication(String token) {
		...
		UserDetails userDetails = org.springframework.security.core.userdetails.User.withUsername(claims.getSubject())
				.password(claims.getSubject()).roles(role).build();
